...
void acceptconn(int lfd,int events,void *arg)
{
    ...
    do
    {
        for(i = 1; i < MAX_EVENTS; i++) //从全局数组g_events中找一个空闲元素，类似于select中找值为-1的元素
        {
...
void input_char(int fd, int events, void *arg) {
	char *input_message = new char[1024];
	bzero(input_message, 1024);
	fgets(input_message, 1024, stdin);
	printf ("boardcast message: [\n%s]\n", input_message);
	for (int i = 1; i < MAX_EVENTS; i++) {
		if (g_events[i].status == 0) {
			continue;
		}
		send(g_events[i].fd, input_message, 1024, 0);
	}
